...
def vis_seg(data, result, img_norm_cfg, data_id, colors, score_thr, save_dir):
    ...
    class_names = ['1_abnormal', '2_abnormal', '3_abnormal']
    ...
    for img, img_meta, cur_result in zip(imgs, img_metas, result):
        ...
        for idx in range(num_mask):
            ...
            cv2.putText(seg_show, label_text, vis_pos,
                        cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255))  # green
        # mmcv.imwrite(seg_show, '{}/{}.jpg'.format(save_dir, data_id))
        mmcv.imwrite(seg_show, '{}/{}'.format(save_dir, data_id))
...
def single_gpu_test(model, data_loader, args, cfg=None, verbose=True):
    ...
    img_name_list = os.listdir("/public/ttzhang9/dataset/us2/resize_us_test_v2")

    prog_bar = mmcv.ProgressBar(len(img_name_list))
    for i, data in enumerate(data_loader):
        ...
        if verbose:
            vis_seg(data, seg_result, cfg.img_norm_cfg, data_id=img_name_list[i], colors=colors, score_thr=args.score_thr, save_dir=args.save_dir)
