...
from .pvt_util import DropPath, to_2tuple, trunc_normal_
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.gcnet1 = ContextBlock(64, 1./4)
        self.gcnet2 = ContextBlock(128, 1. / 4)
        self.gcnet3 = ContextBlock(320, 1. / 4)
        self.conv1 = nn.Conv2d(320, 128, 1, padding=0)
        self.conv2 = nn.Conv2d(320, 64, 1, padding=0)

    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            logger = get_root_logger()
            load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=logger)
        normal_init(self.conv1, std=0.001)
        normal_init(self.conv2, std=0.001)
    ...
    def forward_features(self, x):
        ...
        for blk in self.block2:
            x = blk(x, H, W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # another = F.interpolate(outs[0], scale_factor=0.5, mode='bilinear')
        # x = torch.cat([x, another], dim=1)
        another2 = x
        # another2 = self.gcnet2(another2)
        # print("x2", x.shape)
        outs.append(x)
        ...
        for blk in self.block3:
            x = blk(x, H, W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # another = F.interpolate(outs[1], scale_factor=0.5, mode='bilinear')
        # x = torch.cat([x, another], dim=1)
        another3 = x
        # another3 = self.gcnet3(another3)
        # print("x3", x.shape)
        outs.append(x)
        ...
        for blk in self.block4:
            x = blk(x, H, W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # another = F.interpolate(outs[2], scale_factor=0.5, mode='bilinear')
        # x = torch.cat([x, another], dim=1)
        # another4 = x
        # print("x4", x.shape)
        outs.append(x)
        ...
        # outs[1] = outs[1] + F.interpolate(self.conv1(another3), scale_factor=2, mode='bilinear')
        # aa = F.interpolate(self.conv2(another3), scale_factor=2, mode='bilinear')
        # outs[0] = outs[0] + F.interpolate(aa, scale_factor=2, mode='bilinear')

        # another1 = F.interpolate(another1, scale_factor=0.5, mode='bilinear')
        # outs[1] = torch.cat([outs[1], another1], dim=1)
        # another2 = F.interpolate(another2, scale_factor=0.5, mode='bilinear')
        # outs[2] = torch.cat([outs[2], another2], dim=1)
        # another3 = F.interpolate(another3, scale_factor=0.5, mode='bilinear')
        # outs[3] = torch.cat([outs[3], another3], dim=1)
        # print(outs[0].shape)
        # print(outs[1].shape)
        # print(outs[2].shape)
        # print(outs[3].shape)
