...
class SOLOv2Head(nn.Module):
    ...
    def loss(self,
        ...
        # print(len(ins_label_list), len(cate_label_list), len(ins_ind_label_list), len(grid_order_list))#  2222
        ...
        for b_kernel_pred in kernel_preds:
            ...
            for idx, kernel_pred in enumerate(b_kernel_pred):
                ...
                # print("cur_ins_max", cur_ins_pred.max())# 10
                # print("cur_ins_min", cur_ins_pred.min())#-27...
    ...
    def solov2_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            print((lower_bound, upper_bound)) # ((1, 56), (28, 112), (56, 224), (112, 448), (224, 896))
            print("hit_indices", hit_indices) #tensor([0], device=cuda)huozhe []
            print("num_grid", num_grid) #from [40, 36, 24, 16, 12]
            ...
            print("gt_bboxes", gt_bboxes) # gt_bboxes tensor([[ 306.7826, 253.9131, 379.8261, 295.6522]], device='cuda:0')
            print("self.sigma", self.sigma) # 0.2
            print("mask_feat", mask_feat_size[0], mask_feat_size[1]) # example 144 120
            gt_labels = gt_labels_raw[hit_indices] # 选是1，2，3
            ...
            for seg_mask, gt_label, half_h, half_w, center_h, center_w, valid_mask_flag in zip(gt_masks, gt_labels,
                ...
                if not valid_mask_flag:
                    continue
                print("gt_label", gt_label)# (1, 56)
                print("half_h, half_w, center_h, center_w", half_h, half_w, center_h, center_w)
                # exmaple tensor(4.1739, device='cuda:0') tensor(7.3043, device='cuda:0') tensor(275.6954, device='cuda:0') tensor(345.8336, device='cuda:0')

                upsampled_size = (mask_feat_size[0] * 4, mask_feat_size[1] * 4) # e 480, 544
                coord_w = int((center_w / upsampled_size[1]) // (1. / num_grid)) # //表示除后去除小数位 10
                coord_h = int((center_h / upsampled_size[0]) // (1. / num_grid))
                print("coord_w, coord_h", coord_w, coord_h) # 27 22
                ...
                print("top_box, down_box, left_box, right_box", top_box, down_box, left_box, right_box) # 22 23 26 27

                top = max(top_box, coord_h - 1)
                down = min(down_box, coord_h + 1)
                left = max(coord_w - 1, left_box)
                right = min(right_box, coord_w + 1)
                print("top, down, left, right", top, down, left, right)# 22, 23 ,26, 27

                cate_label[top:(down + 1), left:(right + 1)] = gt_label
                print("seg_mask_before", seg_mask.shape) # 480, 512
                seg_mask = mmcv.imrescale(seg_mask, scale=1. / output_stride)
                print("seg_mask_after", seg_mask.shape) # 120,128 缩小4倍
                ...
                for i in range(top, down + 1):
                    for j in range(left, right + 1):
                        ...
                        cur_ins_label = torch.zeros([mask_feat_size[0], mask_feat_size[1]], dtype=torch.uint8,
                                                    device=device)
                        print("cur_ins_label", cur_ins_label.shape) # e 120 128
            ...
            ins_label_list.append(ins_label)  #####mask
            cate_label_list.append(cate_label)
            ins_ind_label_list.append(ins_ind_label)
