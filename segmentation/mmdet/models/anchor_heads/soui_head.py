...
class SOUIHead(nn.Module):
    ...
    def loss(self,
        ...
        for input, target in zip(ins_pred_list, ins_labels):
            ...
            loss_bin = F.binary_cross_entropy(input, target.float(), reduction='none')
            loss_bin = loss_bin.view(input.size()[0], -1)
            loss_bin = torch.mean(loss_bin, dim=1)
            loss_ce.append(loss_bin)
        loss_ins = torch.cat(loss_ins).mean()
        loss_ce = torch.cat(loss_ce).mean()
        loss_ins = loss_ins * self.ins_loss_weight + loss_ce * 5.0
