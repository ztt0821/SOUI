...
from block import RRDB, conv_block
...
class Model(nn.Module):
    def __init__(self, in_channel=3, feature_channel=64, s_channel=8, out_channel=3, s1_ch=64, s2_ch=32, s3_ch=16, s4_ch=8):
        ...
        ########background
        self.grad_fea = conv_block(in_channel, feature_channel, kernel_size=3, norm_type=None, act_type=None)
        self.RRDB1 = RRDB(feature_channel*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
            norm_type=None, act_type='leakyrelu', mode='CNA')
        self.concat_1 = conv_block(feature_channel*2, feature_channel, kernel_size=3, norm_type=None, act_type=None)
        self.RRDB2 = RRDB(feature_channel*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
            norm_type=None, act_type='leakyrelu', mode='CNA')
        self.concat_2 = conv_block(feature_channel * 2, feature_channel, kernel_size=3, norm_type=None, act_type=None)
        self.RRDB3 = RRDB(feature_channel*2, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
            norm_type=None, act_type='leakyrelu', mode='CNA')
        self.concat_3 = conv_block(feature_channel * 2, feature_channel, kernel_size=3, norm_type=None, act_type=None)
        self.grad_out = outconv(feature_channel, out_channel)

        self.conv1 = nn.Sequential(
            single_conv(s1_ch+16, s1_ch)
        )
        self.conv2 = nn.Sequential(
            single_conv(s2_ch+16, s2_ch)
        )
        self.conv3 = nn.Sequential(
            single_conv(s3_ch+16, s3_ch)
        )
        self.conv4 = nn.Sequential(
            single_conv(s4_ch+16, s4_ch)
        )
    ...
    def forward(self, x):
        # x inchannel =3=grad  x 64 grad_1 64
        grad = self.get_grad(x)
        x = self.head(x)
        # print("x",x.shape)
        grad_1 = self.grad_fea(grad)
        grad_1_cat = torch.cat([x, grad_1], dim=1)
        grad_2 = self.RRDB1(grad_1_cat)
        grad_2 = self.concat_1(grad_2)
        ...
        # print("wx2", wx2.shape)
        grad_2_cat = torch.cat([wx2, grad_2], dim=1)
        # print("grad_2", grad_2.shape)
        grad_3 = self.RRDB2(grad_2_cat)
        grad_3 = self.concat_2(grad_3)

        inx1, u1, out1 = self.unet(wx1)
        inx2, u2, out2 = self.unet(wx2)
        # print("inx2", inx2.shape)
        # print("u2", u2.shape)
        # print("grad_3", grad_3.shape)
        grad_3_cat = torch.cat([u2, grad_3], dim=1)
        grad_4 = self.RRDB3(grad_3_cat)
        grad_4 = self.concat_3(grad_4)

        s1 = self.sptblk(u1)
        out3 = self.outc3(s1)

        grad_4_1, grad_4_2, grad_4_3, grad_4_4 = torch.chunk(grad_4, 4, dim=1)
        # print("grad_4_1", grad_4_1.shape)
        u2_new = torch.cat([u2, grad_4_1], dim=1)
        spt1 = self.conv1(u2_new)
        spt1_1, spt1_2 = torch.chunk(spt1, 2, dim=1)
        spt1_2_new = torch.cat([spt1_2, grad_4_2], dim=1)
        spt2 = self.conv2(spt1_2_new)
        # print("spt2", spt2.shape)
        spt2_1, spt2_2 = torch.chunk(spt2, 2, dim=1)
        spt2_2_new = torch.cat([spt2_2, grad_4_3], dim=1)
        spt3 = self.conv3(spt2_2_new)
        spt3_1, spt3_2 = torch.chunk(spt3, 2, dim=1)
        spt3_2_new = torch.cat([spt3_2, grad_4_4], dim=1)
        spt4 = self.conv4(spt3_2_new)
        s2 = self.conv5(spt1_1) + self.conv6(spt2_1) + spt3_1 + spt4

        # s2 = self.sptblk(u2)
        out4 = self.outc4(s2)

        grad_out = self.grad_out(grad_4)
        '''
        ...
        '''
        return lx1, lx2, out1, out2, out3, out4, grad_out
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return inx, conv4, out  # conv4（channel=64)，out（channel=3)
...
if __name__ == '__main__':
    ...
    # model_net = SptBlk()
    # output = model_net(input2)
    # print(output.shape)

    # out1, out2 = torch.chunk(input2, 2, dim=1)
    # print(out1.shape)
    # print(out2.shape)
