...
class TEST_CT_DATASET(data.Dataset):
    def __init__(self, dataPath='', loadSize=286, fineSize=256, flip=1):
        super(TEST_CT_DATASET, self).__init__()
        # list all images into a list
        # self.listA = [x for x in listdir(dataPath + '/resize_us_test/') if is_image_file(x)]
        self.listA = [x for x in listdir(dataPath + '/resize_us/') if is_image_file(x)]
        # self.listB = [x for x in listdir(dataPath + '/B/') if is_image_file(x)]
        # self.listR = [x for x in listdir(dataPath+'/R/') if is_image_file(x)]
    ...
    def __getitem__(self, index):
        # 1. Read one data from file (e.g. using numpy.fromfile, PIL.Image.open).
        pathA = os.path.join(self.dataPath + '/resize_us/', self.listA[index])
        # pathA = os.path.join(self.dataPath + '/resize_us_test/', self.listA[index])
        # pathB = os.path.join(self.dataPath + '/B/', self.listB[index])
        # pathR = os.path.join(self.dataPath+'/R/',self.listR[index])
        ...
        return imgA, self.listA[index]

    def __len__(self):
        # You should change 0 to the total size of your dataset.
        return len(self.listA)

class TEST_CT_DATASET_EVAL(data.Dataset):
    def __init__(self, dataPath='', loadSize=286, fineSize=256, flip=1):
        super(TEST_CT_DATASET_EVAL, self).__init__()
        # list all images into a list
        # self.listA = [x for x in listdir(dataPath + '/resize_us_test/') if is_image_file(x)]
        self.listA = [x for x in listdir(dataPath + '/save_noisy/') if is_image_file(x)]
        self.listB = [x for x in listdir(dataPath + '/save_clean/') if is_image_file(x)]
        # self.listR = [x for x in listdir(dataPath+'/R/') if is_image_file(x)]
    ...
    def __getitem__(self, index):
        # 1. Read one data from file (e.g. using numpy.fromfile, PIL.Image.open).
        pathA = os.path.join(self.dataPath + '/save_noisy/', self.listA[index])
        # pathA = os.path.join(self.dataPath + '/resize_us_test/', self.listA[index])
        pathB = os.path.join(self.dataPath + '/save_clean/', self.listB[index])
        # pathR = os.path.join(self.dataPath+'/R/',self.listR[index])
        ...
        return imgA, self.listA[index], imgB
